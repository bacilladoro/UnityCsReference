...
#pragma warning restore UA2001
        ...
        private void ShowGeneral(string searchContext)
        {
            ...
            if (m_EnableEditorAnalytics != s_OriginalEnableEditorAnalytics)
                EditorGUILayout.HelpBox(GeneralPropertiesNonSearchable.changingThisSettingRequiresRestart.text, MessageType.Warning);
        ...
        private void DrawPackageManagerOptions()
        {
            ...
            using (new EditorGUI.DisabledScope(isLogLevelOverridden))
            {
                ...
                {
                    EditorGUILayout.HelpBox(GeneralPropertiesNonSearchable.packageManagerLogLevelOverridden.text, MessageType.Info, true);
                }
            }
        }
        ...
        private void ShowGICache(string searchContext)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                {
                    GUIContent warning = EditorGUIUtility.TextContent(GICachePropertiesNonSearchable.cantChangeCacheSettings.text);
            ...
            using (new EditorGUI.DisabledScope(Lightmapping.isRunning))
            {
                ...
                {
                    EditorUtility.DisplayProgressBar(GICacheProperties.cleanCache.text, GICachePropertiesNonSearchable.pleaseWait.text, 0.0F);
                    Lightmapping.Clear();
                    EditorUtility.DisplayProgressBar(GICacheProperties.cleanCache.text, GICachePropertiesNonSearchable.pleaseWait.text, 0.5F);
        ...
        private void ShowUIScaling(string searchContext)
        {
            ...
            if (Application.platform == RuntimePlatform.WindowsEditor)
            {
                ...
                if (m_ContentScaleChangedThisSession)
                    EditorGUILayout.HelpBox(UIScalingPropertiesNonSearchable.changingThisSettingRequiresRestart.text, MessageType.Warning);
