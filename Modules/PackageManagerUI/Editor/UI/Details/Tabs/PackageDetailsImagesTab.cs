...
namespace UnityEditor.PackageManager.UI.Internal
{
    internal class PackageDetailsImagesTab : PackageDetailsTabElement
    {
        ...
        protected override void RefreshContent(IPackageVersion version)
        {
            ...
            {
                var thumbnail = new Image().WithClassList("thumbnail", "image");
        ...
        internal void OnImageClicked(Image image, PackageImage changeToImage)
        {
            ...
            var previousMainTexture = m_MainImage.image as Texture2D;
            m_AssetStoreCache.DownloadImageAsync(product.id, url, (retId, texture2d) =>
            {
                ...
                if (retId.ToString() == m_Version.package?.uniqueId)
                {
                    if (previousMainTexture != null
                        && previousMainTexture != s_LoadingTexture
                        && previousMainTexture != texture)
                    {
                        m_ImageTextures.Remove(previousMainTexture);
                        Object.DestroyImmediate(previousMainTexture);
                    }
