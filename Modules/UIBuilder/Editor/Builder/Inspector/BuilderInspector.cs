    ...
    internal sealed class BuilderInspector : BuilderPaneContent, IBuilderSelectionNotifier, IDisposable
    {
        ...
        void UpdateBoundValue(VisualElement field)
        {
            ...
            else
            {
                ...
                else if (id == StylePropertyId.BackdropFilter)
                {
                    var backdropFilterField = field.GetFirstAncestorOfType<FilterStyleField>();
                    styleFields.RefreshBackdropFilterStyleField(backdropFilterField);
                }
