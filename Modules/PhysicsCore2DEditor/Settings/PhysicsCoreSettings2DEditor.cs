...
namespace Unity.U2D.Physics.Editor
{
    ...
    sealed class PhysicsCoreSettings2DEditor : UnityEditor.Editor
    {
        ...
        static class Tooltips
        {
            ...
            public const string initialWorldCapacity = "Controls the number of worlds allocated up-front when the physics system starts. The larger the capacity, the more memory that is allocated immediately so care must be taken. When all worlds are in use, more are allocated on demand up to the supported maximum, so this is an initial capacity and not a hard limit. Growing the array reallocates it, so set this to the number of worlds you expect to avoid reallocations during gameplay. Setting this value to one reduces start-up memory usage to a minimum while still allowing more worlds to be created later. Any change will only be actioned by Exiting Play mode in the Editor or restarting the player build. A single PhysicsWorld.defaultWorld is automatically created therefore occupies one of the allocated worlds.";
        ...
        static public VisualElement CreatePropertyGUI(SerializedObject serializedObject)
        {
            ...
            void SetupTabGlobal(VisualElement root)
            {
                ...
                root.Add(CreateInspectorPropertyField(nameof(PhysicsCoreSettings2D.m_InitialWorldCapacity), Tooltips.initialWorldCapacity, (settings) => settings.initialWorldCapacity = 4, false));
